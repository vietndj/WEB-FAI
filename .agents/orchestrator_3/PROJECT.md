...
- **Component Architecture**: Atomic component decomposition:
  - `src/components/tuyen-sinh/`: Atomic sections for the Admissions page (< 250 lines for `page.js`).
  - `src/components/ve-fai/`: Atomic sections for the About FAI page (< 250 lines for `page.js`).
  - `src/components/course/CourseLayout.jsx`: Reusable layout eliminating ~5,800 lines of duplicate code across all 11 course pages.
...
## Code Layout
- Metadata & Reports: `.agents/orchestrator_3/` (ONLY metadata/plans/progress, no source code).
- Source Code:
  - `fai/src/data/` (programs.js, scholarships.js, tuition.js, contacts.js)
  ...
  - `fai/src/components/course/CourseLayout.jsx`
...
|---|------|-------|-------------|--------|
| M1 | Data Modules & SSoT | Implement `src/data/` modules and refactor shared consumer components | None | IN_PROGRESS |
| M2 | Component Decomposition | Decompose `src/app/tuyen-sinh/page.js` and `src/app/ve-fai/page.js` | M1 | PLANNED |
| M3 | Reusable Course Layout | Implement `CourseLayout.jsx` and refactor 11 course pages | M1 | PLANNED |
| M4 | Design System & CSS Standardization | Implement `fai-design-system.css`, reduce inline styles, ensure responsive | M2, M3 | PLANNED |
...
- `export const TRAINING_PROGRAMS_2026`: Array of `{ brand, brandId, programs: string[] }`
- `export const programsByBrand`: Object keyed by `aptech`, `arena`, `skillking`, `jetking` with brand metadata and array of program objects `{ id, name, fullName, duration, slug, route, semesters, highlights }`
- `export const aptechSwitcherItems`, `arenaSwitcherItems`, `skillkingSwitcherItems`, `jetkingSwitcherItems`: Array of `{ label, href, path }`
- `export const aptechShortCourseOptions`, `arenaSpecializationOptions`, `arenaShortCourseOptions`, `skillkingShortCourseOptions`: string[]

### `src/data/scholarships.js`
- `export const SCHOLARSHIP_BRANDS`: Object keyed by `aptech`, `arena`, `skillking`, `jetking` with `{ id, name, tagline, themeColor, accentBg, borderColor, gradientBg, campuses, items: [{ id, title, value, unit, amount, desc, badge, tags }] }`
- `export const BRAND_FORM_PRESETS`: Object keyed by brand/track with form header titles, subtitles, and badges

### `src/data/tuition.js`
- `export const TUITION_ACCOUNTS`: Array of `{ campusKey, campusName, badge, accountNumber, accountName, bankName, shortBank, branch, transferSyntax, syntaxExample, address }`
- `export const TUITION_TRANSFER_NOTES`: string[]

### `src/data/contacts.js`
- `export const HOTLINES`: `{ hn, dn, primary, secondary }`
- `export const EMAILS`: `{ general, admissions, mailto }`
- `export const WORKING_HOURS`: `{ display, detail }`
- `export const ADMISSION_CAMPUSES`: `['Hà Nội', 'Đà Nẵng']`
- `export const EXTERNAL_LINKS`: `{ privacyPolicy, leadSubmitScript, websites, zalo }`
- `export const aptechCampuses`, `arenaCampuses`, `skillkingCampuses`, `jetkingCampuses`, `CAMPUSES_BY_CITY`

### `src/components/course/CourseLayout.jsx`
- Props: `{ brandKey, themeColor, bgWatermark, brandBadge, title, description, stats, bannerImage, ProgramSwitcher, highlights, semesters, customContent, formProps, ctaTitle, ctaDesc, ctaButtonText, ctaButtonHref }`
